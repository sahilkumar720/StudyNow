...
function Nav() {
  ...
  return (
    <div>
    <div className='w-[100%] h-[70px] fixed top-0 px-[20px] py-[10px] flex items-center justify-between bg-[#00000055]  z-10'>
     <div className='lg:w-[20%] w-[40%] lg:pl-[50px] '>
        <img src={logo} className=' w-[280px]' onClick={()=>navigate("/")} alt="" />
     ...
     <div className='w-[30%] lg:flex items-center justify-center gap-4 hidden'>
       ...
       <div className='w-[50px] h-[50px] rounded-full text-white flex items-center justify-center text-[20px] border-2 bg-black  border-white cursor-pointer' onClick={()=>setShowPro(prev=>!prev)}>
         ...
         :
         <div className='w-[45px] h-[45px] rounded-full text-white flex items-center justify-center text-[21px] border-1 bg-black  border-white cursor-pointer' >{userData?.name.slice(0,1).toUpperCase()}</div>}
          </div>}
           {userData?.role == "educator" ? <div className='px-[20px] py-[7px] border-2 lg:border-white border-black lg:text-white bg-[black] text-black rounded-[6px] text-[18px] font-light flex gap-2 cursor-pointer' onClick={()=>navigate("/dashboard")}>Dashboard</div>
           :""}
        {!userData && <span className='px-[20px] py-[7px] border-2 border-white text-white rounded-[6px] text-[18px] font-light cursor-pointer bg-[#000000d5] ' onClick={()=>navigate("/login")}>Login</span>}
        {userData && <span className='px-[20px] py-[7px] bg-white text-black rounded-[6px] shadow-sm shadow-black text-[18px] cursor-pointer' onClick={handleLogout}>LogOut</span>}
